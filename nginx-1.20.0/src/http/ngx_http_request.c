...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    if (rev->timedout) {
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, rev->log, 0, "AAAAAAAAAAAAAAAAAAAAA");
        ngx_log_error(NGX_LOG_INFO, c->log, NGX_ETIMEDOUT, "client timed out");
        c->timedout = 1;
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            // Custom information about request
    	    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "==========CUSTOM INFO ABOUT REQUEST==========");
    	    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Request URI, server, client, protocol version, method: %V", &r->unparsed_uri);
            ...
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Request headers:");
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0,
                  "Request finalized for %V %V with status %i",
                  &r->method_name, &r->unparsed_uri, rc);
